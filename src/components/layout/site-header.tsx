        ...
        <div className="mt-1 max-h-[32rem] overflow-y-auto">
          {items.map((item) => (
            <Link
              ...
              href={item.href}
              className={`relative flex items-center gap-4 px-3 py-3.5 transition ${isCourseMenu ? 'border-b border-navy-100 last:border-b-0' : 'rounded-xl hover:bg-navy-50'} ${item.highlighted ? 'bg-sky-100 hover:bg-sky-100' : isCourseMenu ? 'hover:bg-navy-50' : ''}`}
            >
              ...
              {item.image ? (
                ...
                <span className="flex h-10 w-10 shrink-0 items-center justify-center text-blue-800" aria-hidden="true"><MenuIcon type={item.icon} /></span>
              ) : null}
              <span className="min-w-0 flex-1">
                <span className={`block font-semibold ${isCourseMenu ? 'text-[15px]' : 'text-sm'}`}>{item.label}</span>
                <span className={`mt-0.5 block leading-5 text-navy-400 ${isCourseMenu ? 'text-[13px]' : 'text-xs'}`}>{item.description}</span>
              </span>
      ...
      <div className="absolute end-0 z-50 mt-2 max-h-[75vh] w-[min(22rem,88vw)] overflow-y-auto rounded-2xl border border-navy-100 bg-white p-3 text-navy-900 shadow-2xl">
        <div className="grid grid-cols-2 gap-2 border-b border-navy-100 pb-3">
          {directItems.map((item) => (
            <Link key={item.href} href={item.href} className="rounded-lg bg-navy-50 px-3 py-2 text-sm font-semibold hover:bg-gold-200/50">{item.label}</Link>
          ))}
        </div>
        {sections.map(([sectionLabel, items]) => (
          <section key={sectionLabel} className="border-b border-navy-100 py-3 last:border-0">
            ...
            <div className="mt-2 grid gap-1">
              {items.map((item) => (
                <Link key={`${item.href}-${item.label}`} href={item.href} className={`rounded-lg px-2 py-2 text-sm hover:bg-navy-50 ${item.highlighted ? 'bg-sky-100' : ''}`}>{item.label}</Link>
              ))}
            </div>
          </section>
        ))}
...
export async function SiteHeader({ locale }: { locale: Locale }) {
  ...
  const courseCopy = {
    en: {
      ...
      remoteDescription: 'Reserve a remote session and access Digital-UNI Zoom and Google Classroom.',
      tutor: 'Digital-UNI AI Tutor (K–12)',
      tutorDescription: '$30/hour · All subjects',
      professional: 'Professional Certification',
      professionalDescription: '12-week programs · Starting at $3,000',
      executive: 'Executive Management',
      executiveDescription: '6-week programs · Starting at $25,000',
      all: 'All Courses',
      allDescription: 'View complete course catalog'
    },
    fr: {
      ...
      remoteDescription: 'Réserver une session à distance et accéder à Zoom et Google Classroom de Digital-UNI.',
      tutor: 'Tuteur IA Digital-UNI (K–12)',
      tutorDescription: '30 $/heure · Toutes les matières',
      professional: 'Certification professionnelle',
      professionalDescription: 'Programmes de 12 semaines · À partir de 3 000 $',
      executive: 'Management exécutif',
      executiveDescription: 'Programmes de 6 semaines · À partir de 25 000 $',
      all: 'Tous les cours',
      allDescription: 'Voir le catalogue complet des cours'
    },
    ar: {
      ...
      remoteDescription: 'حجز جلسة عن بُعد والوصول إلى Zoom وGoogle Classroom الخاصة بـ Digital-UNI.',
      tutor: 'مدرّس Digital-UNI بالذكاء الاصطناعي (K–12)',
      tutorDescription: '30 دولارًا/الساعة · جميع المواد',
      professional: 'الشهادة المهنية',
      professionalDescription: 'برامج 12 أسبوعًا · تبدأ من 3,000 دولار',
      executive: 'الإدارة التنفيذية',
      executiveDescription: 'برامج 6 أسابيع · تبدأ من 25,000 دولار',
      all: 'جميع الدورات',
      allDescription: 'عرض دليل الدورات الكامل'
    }
  }[locale];
