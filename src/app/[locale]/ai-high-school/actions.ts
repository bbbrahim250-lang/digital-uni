  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        reply_to: data.email,
        subject: `Community support — Santa Monica AI High School — ${data.signatureName}`,
        text: [
          message,
          '',
    ...
    if (!response.ok) {
      console.error('campaign_support_email_rejected', { status: response.status });
      return { configured: true, accepted: false };
    }

    const accepted = (await response.json()) as { id?: string };
    return { configured: true, accepted: Boolean(accepted.id) };
  } catch (error) {
    console.error('campaign_support_email_failed', {
